...
post1 = Post.find_or_create_by!(slug: "getting-started-with-rails2static") do |p|
  ...
  p.body = <<~BODY
    ...
    Installation takes about a minute. Add the gem to your <code>Gemfile</code>:

    <code>gem "rails2static"</code>

    Run <code>bundle install</code>, then generate the initializer:

    <code>rails generate rails2static:install</code>

    This creates <code>config/initializers/rails2static.rb</code> with sensible defaults. The most important option is <code>exclude_patterns</code>, which lets you skip routes that shouldn't be in the static output — like admin pages or anything behind authentication.

    To generate your site, run:

    <code>rake rails2static</code>

    That's it. Your static site is now in the <code>_site/</code> directory. You can preview it locally with:

    <code>rake static:preview</code>

    This starts a local server at <code>http://localhost:8000</code> so you can verify everything looks right before deploying.
...
post2 = Post.find_or_create_by!(slug: "deploying-to-cloudflare-pages") do |p|
  ...
  p.body = <<~BODY
    ...
    <code>Root directory</code>: your app's directory (e.g. <code>demo</code> if it's in a subdirectory)
    <code>Build command</code>: <code>bundle install && rake rails2static</code>
    <code>Build output</code>: <code>_site</code>

    That's all the configuration you need. Cloudflare will install your gems, run the rake task, and deploy the <code>_site/</code> directory to its global CDN.
...
post3 = Post.find_or_create_by!(slug: "how-the-crawler-works") do |p|
  ...
  p.body = <<~BODY
    ...
    The process starts with entry paths — by default, just <code>/</code>. The crawler fetches each entry path using <code>Rack::Test</code>, which calls your Rails app directly through the middleware stack without an HTTP server. This means the crawl is fast and doesn't need a running process.

    For each HTML page it fetches, the crawler parses the response with Nokogiri and extracts all internal links: <code>a[href]</code>, <code>link[href]</code>, <code>script[src]</code>, <code>img[src]</code>, and <code>srcset</code> attributes. These discovered URLs get added to a queue.

    The crawler continues until the queue is empty or it hits the <code>max_pages</code> safety limit (default: 10,000). It tracks visited URLs to avoid cycles and skips anything matching your <code>exclude_patterns</code>.

    After crawling, the link rewriter adjusts <code>href</code> attributes so they work as static files. If <code>trailing_slash</code> mode is enabled (the default), <code>/about</code> becomes <code>/about/index.html</code> and links point to <code>/about/</code> — which most static hosts serve correctly.

    Finally, the asset collector fetches all CSS, JavaScript, images, and fonts referenced in your pages. It even parses CSS files for <code>url()</code> references and <code>@import</code> statements, fetching those recursively.

    The result is a self-contained <code>_site/</code> directory with everything needed to serve your site from any static host.
  BODY
...
post4 = Post.find_or_create_by!(slug: "why-use-rails-for-a-static-site") do |p|
  ...
  p.body = <<~BODY
    ...
    Scaffolds are another underrated advantage. Need an admin interface to manage content? Run <code>rails generate scaffold</code> and you have full CRUD in seconds. Exclude the admin routes from the static build with an <code>exclude_patterns</code> config, and your admin interface exists only in development.
...
Page.find_or_create_by!(slug: "about") do |p|
  ...
  p.body = <<~BODY
    ...
    The site you're reading right now was built with Rails and converted to static HTML by running <code>rake rails2static</code>. It's deployed on Cloudflare Pages, which rebuilds it automatically on every push to the GitHub repository.
